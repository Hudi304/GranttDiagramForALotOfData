...
function readJSON(json: string): stringType[] {
    let strArray: stringType[] = []
    let array: any[] = JSON.parse(json)
    for (let i = 0; i < array.length; i++) {
        let obj: stringType = new stringType()
...
function transformer(strArray: stringType[]) {
    let dateArray: dateType[] = []
    strArray.forEach((item) => {
      if(item.date! && item.from! && item.to!){
        let date :string [] = item.date!.split('T')
        let obj: dateType = new dateType(
          item.from,
          new Date(date[0]),
          item.to
        )
...
function compare(x1: dateType, x2: dateType) {
...
function printTableBar(month : number, nrDays : number){
...
function printGanttForMonth(monthArray : dateType[]){
...
function displayGanttDiagramPerMonths(dateArray : dateType[]){
  let arraysPerMoths = new Array()
  //? initing array of arrays for each month
  for(let i =0 ; i<12 ; i++){
    arraysPerMoths.push(new Array<dateType>())
  }
